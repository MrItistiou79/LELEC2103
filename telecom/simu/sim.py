...
def add_delay(chain, x, tau):
    ...
    idx = np.floor(sto_frac * R).astype(int)    # Index of best sample (available in transmitted signal oversampled at TX) depicting the fractional delay

    y = np.concatenate((np.zeros(sto_int), x[idx::R]))  # Received signal with 0's from integer delay and sampling starting at the fractional delay
...
def run_sim(chain):
    ...
    preamble_false_detect = np.zeros(len(SNRs_dB))  # Preamble false detection (found in noise)
    SNR_est_matrix = np.zeros((len(SNRs_dB), chain.n_packets))
    
    # Transmitted signals that are independent of the payload bits
    x_pr = chain.modulate(chain.preamble, chain.osr_tx)     # Modulated signal containing preamble    
    x_sync = chain.modulate(chain.sync_word, chain.osr_tx)  # Modulated signal containing sync_word
    x_noise = np.zeros(chain.payload_len * chain.osr_tx)    # Padding some zeros before the packets
    ...
    for n in range(chain.n_packets):
        ...
        # Transmitted signal
        x_pay = chain.modulate(bits, chain.osr_tx) # Modulated signal with payload
        ...
        start_idx = int(tau * chain.osr_rx * B) + chain.payload_len*chain.osr_rx # Delay + noise in beginning, for STO metric
        ...
        w = (np.random.randn(y.size) + 1j * np.random.randn(y.size)) / np.sqrt(2) # Normalized complex normal vector CN(0, 1)
        ...
        for k, SNR_dB in enumerate(SNRs_dB):
            ...
            # SNR estimation
            noise_power_est = np.mean(np.abs(y_filt[0:chain.payload_len*chain.osr_rx])**2)
            signal_energy_est = np.mean(np.abs(y_filt[-chain.payload_len*chain.osr_rx:])**2)-noise_power_est
            ...
            if detect_idx == None: # Misdetection of preamble
                preamble_misdetect[k] += 1
                errors = 0.5*np.ones(len(bits))
                cfo_hat, tau_hat, detect_idx, start_frame = np.nan, np.nan, np.nan, np.nan
                ...
                if detect_idx < start_idx - 4*R: # False detection of preamble (found in noise)
                    ...
                    preamble_error = True
                elif detect_idx > start_idx + len(chain.preamble)*chain.osr_rx: # Misdetection of preamble (found in packet)
                    preamble_misdetect[k] += 1
                    preamble_error = True
                ...
                if chain.bypass_sto_estimation:
                    if chain.bypass_preamble_detect: # In this case, starting index of preamble already contains sto
                        tau_hat = 0
                    else:
                        tau_hat = sto_idx
                else:
                ...
                if chain.bypass_sto_estimation and chain.bypass_preamble_detect: # In this case, also assume perfect frame syncrhonization
                ...
                bits_hat_pay = bits_hat[start_frame:start_frame+chain.payload_len] # Demodulated payload bits
            ...
            sto_err[k] += ( (start_idx + len(chain.preamble)*chain.osr_rx  + len(chain.sync_word)*chain.osr_rx)/(R*B) - (detect_idx + tau_hat + start_frame*chain.osr_rx)/(R*B) )**2
    ...
    ax.plot(SNR_th, 1 - (1 - BER_th_noncoh)**chain.payload_len, label='AWGN Th. FSK non-coh.')
    ...
    test_name = 'test'
    save_var = np.column_stack((SNRs_dB,SNRs_dB+shift_SNR_out,BER,PER,RMSE_cfo,RMSE_sto,preamble_mis,preamble_false))
