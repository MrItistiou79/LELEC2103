...
class PacketUnwrapper:
    ...
    """
    def __init__(self, key, allowed_senders, authenticate=True):
        self.key = key
    ...
    def unwrap_packet(self, packet):
        ...
        _version, sender, payload_length, serial = PACKET_HEADER.unpack(packet[:HEADER_LEN])
        # Validate correct packet length w.r.t. payload
        if len(packet) != MIN_LEN + payload_length:
            print(len(packet))
            print(MIN_LEN + payload_length)
            raise InvalidPacket("Wrong payload length.")
        if self.authenticate:
            # Validate tag in constant time
            if not bytes_eq(tag_cbc_mac(packet[:-TAG_LEN], self.key), packet[-TAG_LEN:]):
