...
def main():
    ...
    parser.add_argument('--input', action="store", type=str,
                ...
                required=True)
    parser.add_argument('--baudrate', action="store", type=int, default=115200,
                help='Serial device baudrate.')
    parser.add_argument('--key', action="store", type=str,
                help='MAC key, encoded in hex.')
    ...
    for packet in input_stream:
        ...
        if len(packet) < MIN_LEN:
            ...
            continue
        version, sender, payload_length, serial_nb = PACKET_HEADER.unpack(packet[:HEADER_LEN])
        ...
        if args.key is not None:
            if bytes_eq(tag_cbc_mac(packet[:-TAG_LEN], key), packet[-TAG_LEN:]):
                ...
                print(tag_cbc_mac(packet[:-TAG_LEN], k))
                print("-------------------------------------------------------------------")
                print(packet[-TAG_LEN:])
